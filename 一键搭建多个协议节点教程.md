...
【注意】：这个vmess-ws(tls)+Argo节点自动配置了cloudflare的cdn。搭建好后脚本会自动分配1个cloudflare的优选域名，如果域名被墙了或者速度不理想，可以在脚本里面根据提示更换其它优选域名，更换优选域名也可以不用重新搭建，直接在客户端里面将域名更改即可。除了填写优选域名以外，还可以用优选的cloudflare的ip，比如这里的[CloudFlare优质IP（不定期更新）](https://github.com/vpn-wiki/new-pac/blob/master/CloudFlare%E4%BC%98%E8%B4%A8IP) ，操作方法：比如在v2rayN客户端里面将这个节点的地址（address）那一栏改成优选的cloudflare的ip。
...
无需域名。搭建好后有6个节点，3个vless节点，1个vmess节点，1个trojan节点，1个ss节点。

其中有3个节点分别是vmess-ws(tls)+Argo节点、vless-ws(tls)+Argo、trojan-tls+argo节点自动配置了cloudflare的cdn。搭建的过程中可以选择cloudflare的优选域名，如果域名被墙了或者速度不理想，直接在客户端里面将域名更改即可。除了填写优选域名以外，还可以用优选的cloudflare的ip，比如这里的[CloudFlare优质IP（不定期更新）](https://github.com/vpn-wiki/new-pac/blob/master/CloudFlare%E4%BC%98%E8%B4%A8IP) ，操作方法：比如在v2rayN客户端里面将这个节点的地址（address）那一栏改成优选的cloudflare的ip。
